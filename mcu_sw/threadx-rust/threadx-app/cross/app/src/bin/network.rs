...
pub fn do_network(
    ...
    let ssid: &'static str = "Raihan";
    let password = "123456789";

    let broker_ip: core::net::Ipv4Addr = core::net::Ipv4Addr::new(10, 89, 27, 224); 
    ...
    let network = create_tcp_network(ssid, password).expect("Failed to initialize TCP network");
    ...
    let flash_duration = Duration::from_millis(500);

    rgb_led.set_color(255, 0, 0); // Red
    thread::sleep(flash_duration).unwrap();
    ...
    loop {
        ...
        if let Some(ref mut actual_display) = *display_guard {
            handle_subscribe(&mut transport, sub_topic, &mut subscribed, |_, payload| {
                // Clean message (remove quotes and whitespace)
                let msg = core::str::from_utf8(payload)
                    .unwrap_or("<invalid>")
                    .trim_matches(|c: char| c == '\'' || c == '"' || c.is_whitespace());

                defmt::println!("Received cleaned message: '{}'", msg);
                ...
                if msg.len() == 9 {
                    if let (Ok(r), Ok(g), Ok(b)) = (
                        ...
                        defmt::println!("Parsed RGB: R={}, G={}, B={}", r, g, b);
                        rgb_led.set_color(r, g, b);
                    } else {
                        defmt::error!("Failed to parse RGB digits: '{}'", msg);
                    }
                } else {
                    defmt::error!("Invalid RGB string length: '{}'", msg);
                }
        ...
        if let Ok(event) = recv.receive(WaitOption::NoWait) {
            ...
            defmt::println!("Published temperature to {}: {}", pub_topic, temp_str);
